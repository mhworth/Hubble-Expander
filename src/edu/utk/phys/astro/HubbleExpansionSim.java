...
public class HubbleExpansionSim extends View {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		// Calculate the size of the galaxies according to the physical area of the screen
		double density = getContext().getResources().getDisplayMetrics().density;
		double xdpi = getContext().getResources().getDisplayMetrics().xdpi;
		double ydpi = getContext().getResources().getDisplayMetrics().ydpi;
	...
	public void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < numberOfGalaxies; i++) {
			canvas.save();
			canvas.translate((float)(X[i]-0.5)*width + centerX, (float)(Y[i]-0.5)*height + centerY);
			galaxy.draw(canvas);
			canvas.restore();
		}
	}
